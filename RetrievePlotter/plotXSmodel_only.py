...
def plotXS():
    ...
    if len(sys.argv) < 1:
        print('''Usage: python plotXS.py file.root ''')
        sys.exit()
    else:
        fname = sys.argv[1]
        flag  = 1
    ...
    if project == 'neutron':
        ...
        hhh = gPad.DrawFrame(1, 0.01, 7., 10. ,'cross sections in mb')
    elif project == 'gamma' and int(flag) == 1:
        hhh = gPad.DrawFrame(0, 0.01, 200., crossampl[int(Z)] ,'cross sections in mb')
    elif project == 'proton':
    ...
    if int(flag) == 0:
       ...
       fout = os.path.join(plotpath, 'A_' + os.path.splitext(os.path.basename(fname))[0]) + '_0_200MeV.png'
